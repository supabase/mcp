...
In this package's local HTTP server, signed `requestState` used by secret collection, cost confirmation, destructive SQL confirmation, and destructive notebook run confirmation expires after 120 seconds. The secret tool retains a separate 600-second timestamp-based recovery window for fresh calls; this does not extend URL or token validity. Watch restarts invalidate pending state. Restart the server in your MCP client after each change.
